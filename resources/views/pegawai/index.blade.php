...
@section('table')
  <div class="table-responsive mt-5">
    <table class="table table-striped">
      ...
      <tbody>
        @php $filtered = $users; @endphp
        @if(request('role'))
          @php $filtered = $filtered->where('role', request('role')); @endphp
        @endif
        @if(request('created_sort') === 'latest')
          @php $filtered = $filtered->sortByDesc('created_at'); @endphp
        @elseif(request('created_sort') === 'oldest')
          @php $filtered = $filtered->sortBy('created_at'); @endphp
        @endif
        @if(request('sort_name') === 'asc')
          @php $filtered = $filtered->sortBy('name'); @endphp
        @elseif(request('sort_name') === 'desc')
          @php $filtered = $filtered->sortByDesc('name'); @endphp
        @endif
